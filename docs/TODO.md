...
- [x] T0835 Verify rule 1: cop and thief run as two fully separate processes — verified during shared monorepo development via `main.py --role cop|thief`, two independent OS processes (Chapter 2). Since the two-repo submission split, the separation is stronger still: this repo's `main.py serve` only ever runs the cop role at all (no `--role` flag, no thief config present), and the sibling thief repo is the genuinely separate process/repo/codebase-export running the thief side
...
- [ ] T0879 Verify rule 45: team identity is encoded as an 8-character unique code without spaces — **not done**: this repo's own `config/cop/game.toml` still has `group_name = "TBD"` / `group_id = "TBD"` placeholders; a real team code needs to be chosen and propagated before any real match or submission (the sibling thief repo tracks its own `config/thief/game.toml` independently — this repo has no visibility into it)
